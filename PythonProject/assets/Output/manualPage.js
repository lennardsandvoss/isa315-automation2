...
const computeDefaultSlot = (sectionId, index, base={}) => {
  if (!base.sheet || !base.startCell) return null;
  const anchor = parseCellAddress(base.startCell);
...
let LOGGED_LAYOUT_SOURCE = false;
...
const resolveEvidenceLayout = () => {
  ...
  if (!CACHED_EVIDENCE_LAYOUT || signature !== CACHED_LAYOUT_SIGNATURE) {
    ...
    LOGGED_LAYOUT_SOURCE = false;
  }
  if (!LOGGED_LAYOUT_SOURCE) {
    try {
      const details = {
        source: rawLayout?.__source || 'default',
        generatedAt: rawLayout?.__generatedAt || null,
        sections: Object.keys(CACHED_EVIDENCE_LAYOUT?.sections || {} ).length,
      };
      console.info('[ISA315][EvidenceLayout] Using layout', details);
    } catch (_) { /* noop */ }
    LOGGED_LAYOUT_SOURCE = true;
  }
...
const getSectionLayout = (sectionId, layoutArg) => {
  ...
  const sheet = override.sheet || defaults.sheet || null;
  const startCell = override.startCell || defaults.startCell || null;
  ...
  const header = {
    ...
    sheet: headerOverride.sheet || override.sheet || headerDefaults.sheet || defaults.header?.sheet || sheet,
  };
...
const prepareEvidencePayload = async (sections, layoutArg) => {
  ...
  for (const section of sections) {
    ...
    for (let idx = 0; idx < limit; idx++) {
      ...
      if (!file) continue;
      let slot = null;
      const slotCandidate = sectionLayout.slots[idx];
      if (slotCandidate && slotCandidate.sheet && slotCandidate.imageCell) {
        slot = slotCandidate;
      } else {
        const fallback = computeDefaultSlot(section.id, idx, {
        ...
        if (fallback && fallback.sheet && fallback.imageCell) slot = fallback;
      }
      ...
      const entry = {
        ...
        sectionFolder,
        slotIndex: idx,
        slotId: (slot && slot.id) || buildSlotId(section.id, idx),
        targetSlot: slot,
      };
      if (entry.isImage) {
...
const applyEvidenceToWorkbook = (workbook, evidence, layoutArg) => {
  ...
  evidence.forEach(section => {
    ...
    if (header.enabled && header.sheet && header.cell) {
    ...
    section.items.forEach((item, idx) => {
      ...
      if (item.targetSlot && item.targetSlot.sheet && item.targetSlot.imageCell) {
        ...
        const candidate = sectionLayout.slots[idx];
        if (candidate && candidate.sheet && candidate.imageCell) {
          slot = candidate;
        } else {
          const fallback = computeDefaultSlot(section.sectionId, idx, {
            sheet: sectionLayout.sheet,
            startCell: sectionLayout.startCell,
            rowStride: sectionLayout.rowStride,
            imageSize: sectionLayout.imageSize,
      ...
      if (!slot || !slot.sheet || !slot.imageCell) return;
      ...
      const linkAddress = slot.linkCell || (sectionLayout.linkOffset && slot.imageCell
        ? `${numberToColumn(Math.max(1, anchor.col + getColumnOffset(sectionLayout.linkOffset)))}${Math.max(1, anchor.row + getRowOffset(sectionLayout.linkOffset))}`
        : null);
      if (linkAddress) {
      ...
      if (item.isImage && item.base64) {
        ...
        if (slot.imageCell) {
          const fallbackCell = ws.getCell(slot.imageCell);
